...
public class App {
	...
	private static void storeContacts() {
		System.out.println("enter a filename (no space): ");
		String backupFileName = sc.next();
		if (backupFileName == ".ser") {
			backupFileName = new SimpleDateFormat("yyyy-MM-dd-hh-mm-ss").format(new Date()) + ".ser";
		}
	...
	private static void restoreContacts() {
		//list all the file and display them to the user
		int numFiles = 0;
		List<Path> listPaths = new ArrayList<>();
		try (DirectoryStream<Path> ds = Files.newDirectoryStream(Paths.get(BOOK_BKP_DIR), "*.ser")) {
			
			for(Path path : ds) {
				System.out.print((numFiles+1)+": ");
				System.out.println(path.toString().split("\\\\")[3]);
				numFiles++;
				listPaths.add(path);
			}
		...
		//get the file the user wish
		try {
			int fileNum = sc.nextInt()-1;
			if (fileNum>numFiles || fileNum<0) {
				System.out.println("Number does not match a file");
				return;
			}
			//restore
			try (ObjectInputStream ois = new ObjectInputStream(Files.newInputStream(listPaths.get(fileNum)))) {
				book = (Book) ois.readObject();
				System.out.println("Restauration terminée : fichier " + listPaths.get(fileNum).getFileName());
	...
	private static void exportContacts() {
		System.out.println("Quel types d'export vouler vous réaliser.");
		...
		try {
			...
			System.err.println("mauvaise type entrée.\n"
					+ e);
		}
	}

	private static void exportContactsXML() {
	...
	private static void exportContactsCVS() {
		System.out.println("id,Code,contact,");
		System.out.println(book.getId()+","+book.getCode());
		book.getContacts().forEach((k,v)->
